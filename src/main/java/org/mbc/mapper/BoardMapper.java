...
public interface BoardMapper {
	...
	@Select("SELECT * FROM tbl_board WHERE bno > 0") // bno > 0 pk로 선언되어 있어서 인덱싱으로 빠른 추출
	public List<BoardVO> getList(); // 추상메서드
	
	// 그래서 xml를 이용하여 sql문 처리를 진행한다.
	// xml을 이용하려면 src/main/resources폴더 아래쪽에 org 폴더 아래에 mbc 폴더 아래에 mapper 폴더를 만들고
	...
	// 번호가 들어가면 필드 수정 후 몇 개가 수정됐는지 리턴한다.
